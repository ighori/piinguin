...
package com.snowplowanalytics.piinguin.server.implementations
...
class PiinguinImplSpec(implicit ee: ExecutionEnv) extends Specification with FutureMatchers with DynamoDBContext {
  ...
  def e1 = { dbc: DynamoDBClient =>
    ...
    p.createPiiRecord(new CreatePiiRecordRequest(Some(PiiRecord("1234", "5678")))) must be_==(
      new ChangeRecordResponse(Some(Error("OK", true)))).await(retries = 0, timeout = 5 seconds)
  }
  ...
  def e2 = { dbc: DynamoDBClient =>
    ...
    p.createPiiRecord(new CreatePiiRecordRequest(Some(PiiRecord("1234", "9012")))) must be_==(
      new ChangeRecordResponse(Some(Error("Replaces PiiRecord(1234,5678)", true))))
  ...
  def e3 = { dbc: DynamoDBClient =>
    ...
    (result.error must beSome) and
      (result.error.get.isSuccess must beFalse) and
      (result.error.get.message must contain("Cannot do operations on a non-existent table"))
  }
  def e4 = { dbc: DynamoDBClient =>
    ...
    responses.head must be_==(new ChangeRecordResponse(Some(Error("OK", true))))
  }
  ...
  def e8 = { dbc: DynamoDBClient =>
    ...
    p.readPiiRecord(
      ...
      new ReadPiiRecordResponse(Some(PiiRecord("1234", "5678")), Some(Error("OK", true))))
      .await(retries = 0, timeout = 5 seconds)
  }
  ...
  def e10 = { dbc: DynamoDBClient =>
    ...
    requests.onCompleted
    while (!ready) { Thread.sleep(100L) }
    responses.head must be_==(new ReadPiiRecordResponse(Some(PiiRecord("1234", "5678")), Some(Error("OK", true))))
  }
